...
namespace OsEngine.Market.Servers.Plaza
{
    ...
    public class PlazaServerRealization : IServerRealization
    {
        ...
        public void Connect()
        {
            ...
            int occurrences = ((ServerParameterInt)ServerParameters[1]).Value;
            _COD = ((ServerParameterBool)ServerParameters[2]).Value;

            _rateGate = new RateGate(1, TimeSpan.FromMilliseconds(1000 / occurrences));
            ...
            try
            {
                if (!_flagCGate)
                {
                    ...
                    SendLogMessage("Connection to the library environment is successful", LogMessageType.System);
                }
            }
            ...
            {
                SendLogMessage($"Failed to initialize connection. Incorrect arguments may have been passed to the function. Errors - {error}", LogMessageType.Error);
                return;
            }
        ...
        public void Dispose()
        {
            lock (_ConnectionLocker)
            {
                ...
                //closing the publisher | отключаем публишер ордеров
        ...
        /// </summary>
        //private const string ConnectionOpenString = "p2tcp://127.0.0.1:4001;app_name=OsEngine100;name=connectorPrime100";
        ...
        /// В случае, если такое соединение закрылось не штатно, придётся идти в диспетчер задач и мочить P2MQRouter*32. Далее из диска С запускать Роутер через start_router.cmd
        /// </summary>
        private const string ConnectionOpenString = "p2lrpcq://127.0.0.1:4001;app_name=OsEngine;timeout=2000;local_timeout=500;name=connectorPrime";
                    ...
                    lock (_ConnectionLocker)
                    {
                            ...
                            _statusNeeded == ServerConnectStatus.Connect)
                        {
                            try
                            {
                                ...
                                SendLogMessage("The connection to the router is successful", LogMessageType.System);
                        ...
                        CheckOrderSender(); // publisher | публишер 

                        CheckListnerInfo(); // Directory of traded instruments | Список торгуемых инструментов
            ...
            try
            {
                ...
                if (_listenerOrderAndMyDeal.State == State.Closed)
                {
                    if (_listenerOrderAndMyDealReplState == null)
                    {
                        ...
                        _listenerOrderAndMyDeal.Open("mode=online");
                    }
                    else
                    {
        ...
        private void CheckOrderSender()
        {
            ...
            catch (Exception error)
            {
                ...
                SendLogMessage($"Publisher error. Error - {error}", LogMessageType.System);
            ...
            try
            {
                switch (msg.Type)
                {
                    ...
                    case MessageType.MsgP2ReplOnline:
                        {
                            ...
                            SendLogMessage(OsLocalization.Market.Message80, LogMessageType.System);
                    ...
                    case MessageType.MsgOpen:
                        {
                            SendLogMessage(OsLocalization.Market.Message79, LogMessageType.System);
                            break;
                        }
                    default:
                        {
            ...
            lock (_publisherLocker)
            {
                ...
                try
                {
                    if (_publisher == null)
                    {
                        if (MyOrderEvent != null)
                        {
                            SendLogMessage("publisher == null", LogMessageType.System);
        ...
        public void ChangeOrderPrice(Order order, decimal newPrice)
        {
            lock (_publisherLocker)
            {
                ...
                try
                {
                    ...
                    PlazaChangePriceOrder changePriceOrder = new PlazaChangePriceOrder();
        ...
        private List<PlazaChangePriceOrder> _changePriceOrders = new List<PlazaChangePriceOrder>();
                            ...
                            try
                            {
                                ...
                                else if (msgData.MsgId == 176)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        SendLogMessage($"Заявка изменена. Message - {msgData["message"].asString()}", LogMessageType.System);
                                    }
                                ...
                                if (msgData.MsgId == 99)
                                {
                                    ...
                                    Thread.Sleep(msgData["penalty_remain"].asInt());
                                    return 0;
                                }
        ...
        public void SendOrder(Order order)
        {
            lock (_publisherLocker)
            {
                ...
                try
                {
                    ...
                    SendLogMessage("The order has been sent - " + DateTime.Now.ToString() + DateTime.Now.Millisecond, LogMessageType.System);
                }
                catch (Exception error)
                {
                    SendLogMessage(error.ToString(), LogMessageType.Error);
